...
class MaxDiffMinManipulation(manipulation.Manipulation):
    def __init__(self, path, dataset, col):
        self.__path = path
        self.__dataset = dataset
        self.__col = col

        super(MaxDiffMinManipulation, self).__init__()

    def __repr__(self):
        return "MaxDiffMin {} {}".format(self.__path, self.__col)
    ...
    def meta_feature_name():
        return [
            "MaxDiffMin-Constant",
            "MaxDiffMin-NotDeeperCount",
            "MaxDiffMin-ToManyMeta"
        ]
...
class MaxDiffMinCandidate(manipulation_candidate.ManipulationCandidate):
    def search(self, path, dataset):
        ...
        for col in dst_table.df.columns:
            ...
            if ftype == feature_types.time:
                ret.append(MaxDiffMinManipulation(
                    path, dataset, col
                ))
        return ret
