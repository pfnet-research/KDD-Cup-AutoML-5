...
class MaxManipulation(aggregation.AggregationManipulation):
    def __init__(self, path, dataset, col):
        super(MaxManipulation, self).__init__(
            path, dataset, col, "Max", "max", "max", False)

    def calculate_priority(self):
        return 0.9 + 0.5 * self.path.not_deeper_count \
...
class MaxCandidate(manipulation_candidate.ManipulationCandidate):
            ...
            for col in dst_table.df.columns:
                ...
                if ftype == feature_types.numerical \
                   ...
                   or ftype == feature_types.t_processed_numerical:
                    ret.append(MaxManipulation(path, dataset, col))
            return ret
        else:
            return []
