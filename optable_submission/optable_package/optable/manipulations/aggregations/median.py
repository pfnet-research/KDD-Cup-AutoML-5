...
class MedianManipulation(aggregation.AggregationManipulation):
    def __init__(self, path, dataset, col):
        super(MedianManipulation, self).__init__(
            path, dataset, col, "Median", "mean", "median", False)
    ...
    def meta_feature_name():
        return [
            "Median-Constant",
            "Median-NotDeeperCount",
            "Median-ToManyMeta"
        ]


class MedianCandidate(manipulation_candidate.ManipulationCandidate):
            ...
            for col in dst_table.df.columns:
                ...
                if ftype == feature_types.numerical \
                   ...
                   or ftype == feature_types.t_processed_numerical:
                    ret.append(MedianManipulation(path, dataset, col))
            return ret
        else:
            return []
