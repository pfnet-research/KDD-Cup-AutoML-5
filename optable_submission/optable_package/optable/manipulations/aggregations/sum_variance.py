...
class SumVarianceManipulation(aggregation.AggregationManipulation):
    def __init__(self, path, dataset, col):
        super(SumVarianceManipulation, self).__init__(
            path, dataset, col, "SumVariance", "variance", "sum", False)

    def calculate_priority(self):
        return 1.8 + 0.5 * self.path.not_deeper_count \
            + 0.6 * self.path.substance_to_many_count(self.dataset, self.col) \
    ...
    def meta_feature_name():
        return [
            "SumVariance-Constant",
            "SumVariance-NotDeeperCount",
            "SumVariance-ToManyMeta"
        ]


class SumVarianceCandidate(manipulation_candidate.ManipulationCandidate):
            ...
            for col in dst_table.df.columns:
                ...
                if ftype == feature_types.numerical \
                   ...
                   or ftype == feature_types.t_processed_numerical:
                    ret.append(SumVarianceManipulation(path, dataset, col))
            return ret
        else:
            return []
