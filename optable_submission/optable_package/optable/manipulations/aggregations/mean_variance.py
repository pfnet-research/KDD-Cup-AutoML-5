...
class MeanVarianceManipulation(aggregation.AggregationManipulation):
    def __init__(self, path, dataset, col):
        super(MeanVarianceManipulation, self).__init__(
            path, dataset, col, "MeanVariance", "variance", "mean", False)
    ...
    def meta_feature_name():
        return [
            "MeanVariance-Constant",
            "MeanVariance-NotDeeperCount",
            "MeanVariance-ToManyMeta"
        ]


class MeanVarianceCandidate(manipulation_candidate.ManipulationCandidate):
            ...
            for col in dst_table.df.columns:
                ...
                if ftype == feature_types.numerical \
                   ...
                   or ftype == feature_types.t_processed_numerical:
                    ret.append(MeanVarianceManipulation(path, dataset, col))
            return ret
        else:
            return []
